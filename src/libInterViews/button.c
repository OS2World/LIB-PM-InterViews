...
inline long HalfRadioButtonSize (long h) { return round(.4*h); }
inline long HalfCheckBoxSize (long h) { return round(.4*h); }
...
ButtonState::ButtonState (long v) {
    value = (void*)v;
    Reference();
}
...
void ButtonState::SetValue (long v) {
    Modify((void*)v);
}
...
void Button::SetDimensions (long width, long height) {
    shape->width = width;
    shape->height = height;
    shape->Rigid();
}
...
PushButton::PushButton (
    const char* str, ButtonState* s, long v, Painter* out
) : (str, s, (void*)v, out) {
    Init();
}
...
void PushButton::Refresh () {
    ...
    r = min(10L*pixels, min(xmax+1, ymax+1)/6L);
    x[0] = 0; y[0] = r;
    x[1] = 0; y[1] = r + r;
    x[2] = 0; y[2] = ymax - r - r;
    x[3] = 0; y[3] = ymax - r;
...
void RadioButton::Redraw (Coord x1, Coord y1, Coord x2, Coord y2) {
    long h = output->GetFont()->Height();
    long r = radio_plain_width;
    output->ClearRect(canvas, x1, y1, x2, y2);
    Coord tx = r + sep;
    Coord ty = (ymax + 1 - h) / 2;
    output->Text(canvas, text, tx, ty);
...
CheckBox::CheckBox (
    const char* str, ButtonState* s, long on, long off, Painter* out
) : (str, s, (void*)on, out) {
    Init((void*)off);
}
