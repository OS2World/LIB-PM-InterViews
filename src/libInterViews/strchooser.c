...
StringChooser::StringChooser (
    ButtonState* bs, long r, long c, const char* s, Alignment a
) : (bs, nil, a) { 
...
void StringChooser::Select (long index) {
    if (index < 0) {
...
void StringChooser::Select (long left, long right) { _sedit->Select(left, right);}
