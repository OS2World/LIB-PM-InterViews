...
GrowingVertices::GrowingVertices (
    ...
    x = new Coord[bufsize];
    y = new Coord[bufsize];
    bcopy(px, x, (int)n*sizeof(Coord));
    bcopy(py, y, (int)n*sizeof(Coord));
}
...
void GrowingVertices::GetOriginal (Coord*& px, Coord*& py, long& n) {
    Copy(x, y, origbufsize, px, py);
    n = origbufsize;
}

void GrowingVertices::GetCurrent (Coord*& px, Coord*& py, long& n) {
...
void GrowingVertices::CheckBufs () {
    if (count + 1 > bufsize) {
        ...
        bcopy(x, nx, (int)count*sizeof(Coord));
        bcopy(y, ny, (int)count*sizeof(Coord));
        delete x;
        delete y;
        x = nx;
        y = ny;
...
void GrowingVertices::Copy (Coord* x, Coord* y, long n, Coord*& nx,Coord*& ny) {
    nx = new Coord[n];
    ny = new Coord[n];
...
void GrowingVertices::DrawVertices (Coord*, Coord*, long) { }
...
void GrowingMultiLine::DrawVertices (Coord* x, Coord* y, long n) {
...
void GrowingPolygon::DrawVertices (Coord* x, Coord* y, long n) {
    if (n == 2) {
...
void GrowingBSpline::DrawVertices (Coord* x, Coord* y, long n) {
...
void GrowingClosedBSpline::DrawVertices (Coord* x, Coord* y, long n) {
